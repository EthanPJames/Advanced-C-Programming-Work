...
int main(int argc, char **argv)
{
  ...
  if (image_data == NULL)
  {
    //free_BMP_image(image_data); //Causes error but it shouldnt because you should free the data if file mallocs but does not read properly
  ...
  {
    new_image = convert_24_to_16_BMP_image(image_data);
  }
  else
  {
